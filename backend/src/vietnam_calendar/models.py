...
class Feed(Base, TimestampMixin):
    __tablename__="feeds"; __table_args__=(CheckConstraint("fetch_interval_minutes BETWEEN 5 AND 1440", name="ck_feeds_interval"), CheckConstraint("trust_score IS NULL OR trust_score BETWEEN 0 AND 100", name="ck_feeds_trust"), CheckConstraint("consecutive_failures >= 0", name="ck_feeds_failures"),CheckConstraint("version >= 1",name="ck_feeds_version"))
...
class Event(Base,TimestampMixin):
    __tablename__="events"; __table_args__=(CheckConstraint("importance_score IS NULL OR importance_score BETWEEN 0 AND 100",name="ck_events_importance_score"),CheckConstraint("version >= 1",name="ck_events_version"),CheckConstraint("NOT must_include OR (must_include_reason IS NOT NULL AND length(trim(must_include_reason)) > 0)",name="ck_events_must_reason"),CheckConstraint("relevance_status <> 'out_of_scope' OR (importance_level IS NULL AND importance_score IS NULL)",name="ck_events_outscope_importance"),CheckConstraint("merged_into_event_id IS NULL OR publication_status = 'hidden'",name="ck_events_merged_hidden"),ForeignKeyConstraint(["current_revision_id","id"],["event_revisions.id","event_revisions.event_id"],name="fk_events_current_revision_same_event",use_alter=True,ondelete="RESTRICT"),Index("ix_events_calendar", "publication_status", "event_date", postgresql_where=text("merged_into_event_id IS NULL")))
